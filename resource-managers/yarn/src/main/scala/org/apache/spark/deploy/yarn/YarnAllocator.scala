...
private[yarn] class YarnAllocator(
  ...
  def allocateResources(): Unit = synchronized {
    logInfo("resource request heartbeat")
  ...
  private def createContainerRequest(
    ...
    new ContainerRequest(resource, nodes, racks, RM_REQUEST_PRIORITY, 0, true, labelExpression.orNull, executionTypeRequest)
  }
  ...
  def handleAllocatedContainers(allocatedContainers: Seq[Container]): Unit = {
    ...
    logInfo("Received %d containers from YARN, launching executors on %d of them. total running containers %d"
      .format(allocatedContainers.size, containersToUse.size,  numExecutorsRunning))
  }
  ...
  private def runAllocatedContainers(containersToUse: ArrayBuffer[Container]): Unit = {
    for (container <- containersToUse) {
      ...
      if (numExecutorsRunning < targetNumExecutors) {
        if (launchContainers) {
          logInfo(s"try to launch $containerId")
